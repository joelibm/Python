...
def report_split(dealerHand,dealerTotal,player,playerHand,playerTotal,playerHand2,playerTotal2):
    print ("Dealer's Hand:",dealerHand,sep='')
    print ("Value:",dealerTotal)
    print (player,"'s First Hand:",playerHand,sep='')
    print ("Value:",playerTotal)
    print (player,"'s Second Hand:",playerHand2,sep='')
    print ("Value:",playerTotal2)
...
def play_hand(player,bet):
    ...
    if dealerTotal == 21:#handles dealer blackjacks
        ...
        return moneyChange
##start split
    elif playerTotal == 2 * firstCard:
        report(player,hiddenDealerTotal,playerTotal,hiddenDealerHand,playerHand)
        splitHand = input("Would you like to split your hand? (y/n)")
        if splitHand=="" or splitHand.lower()[0]=="y":
            playerTotal = firstCard
            playerHand, playerHand2 = playerHand.split(' ')
            playerAce /= 2
                            
            playerAce2 = playerAce
            playerTotal2 = firstCard

            playerHand,playerAce,playerTotal = deal(deck,playerHand,playerAce,playerTotal)
            playerHand2,playerAce2,playerTotal2 = deal(deck,playerHand2,playerAce2,playerTotal2)
            
            report_split(dealerHand,dealerTotal,player,playerHand,playerTotal,playerHand2,playerTotal2)
                    ...
                    if move.lower()[0] == "s":
                        while True:
                            if 9 <= playerTotal <= 11 and move != "s": #allows player to double down
                                ...
                                if doubleDown=="" or doubleDown.lower()[0]=="y":
                                    moneyChange *= 2
                                    deck,playerHand2,playerAce2,playerTotal2,busted = player_hit(deck,player,playerHand2,playerAce2,playerTotal2,hiddenDealerTotal,hiddenDealerHand)
                                    move = "s"
                            else:
                                move = input("Move? (hit/stand)")
                            if move != "" and (move.lower()[0] == ("h") or move.lower()[0] == ("s")):
                                while move.lower()[0] == "h":
                                    ...
                                    move = input("Move? (hit/stand)")
                                if move.lower()[0] == "s":
                                    moneyChange = dealer_split_play(deck,dealerHand,dealerAce,dealerTotal,player,playerHand,playerTotal,moneyChange,playerHand2,playerTotal2)
                                return moneyChange
                                break
                        break
            #break
...
def dealer_split_play(deck,dealerHand,dealerAce,dealerTotal,player,playerHand,playerTotal,moneyChange,playerHand2,playerTotal2):
    dealer_drew = False
    if dealerTotal < 17:
        print ("Dealer's Hand:",dealerHand,sep='')
        print ("Value:",dealerTotal)
